...
void setup() {
  ...
  SerialBT.begin("ESP32_EMG_Sensor");
  while (!Serial) { }
  ...
  if (!SD.begin(CS_PIN)) {
    ...
    delay(1000);
    ESP.restart(); // Restart the ESP32 if SD card fails
  }
  Serial.println("SD card ready to use!");

  // Open or create the file
  EMGD = SD.open("/EMGData02.csv", FILE_WRITE);
  if (!EMGD) {
    Serial.println("Error opening EMGData001.csv");
    delay(1000);
    ESP.restart(); // Restart the ESP32 if file can't be opened
  } else {
    EMGD.println("Timestamp (ms), Signal, Envelope"); // Header for the file
    EMGD.flush(); // Ensure header is written
  }

  Serial.println("Bluetooth device started: ESP32_EMG_Sensor");
}
...
void loop() {
  ...
  if (timer < 0) {
    ...
    if (EMGD) {
      ...
      // Send data over Bluetooth if a client is connected
      if (SerialBT.hasClient()) {
        String dataString = String(millis()) + "," + String(signal) + "," + String(envelope);
